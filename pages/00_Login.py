...
/* Classe especial para mascarar os caracteres em formato de bolinhas de senha */
.mask-password input {
    -webkit-text-security: disc !important;
    text-security: disc !important;
}

/* Oculta e blinda qualquer contêiner nativo do Streamlit no input */
div[data-testid="stTextInputAdornment"] { display: none !important; width: 0px !important; visibility: hidden !important; }

/* Estilização elegante da nossa chave de alternância */
div[data-testid="stCheckboxToggle"] label p { color: #8AB4F8 !important; font-size: 14px !important; font-weight: 600 !important; }
...
with col_direita:
    ...
    if st.session_state.login_step == 1:
        ...
        # O formulário executa de forma linear e limpa sem condicionais internas aninhadas
        with st.form("form_etapa_1", clear_on_submit=False):
            email = st.text_input("E-mail", placeholder="seu.email@email.com")
            
            st.markdown("<br>", unsafe_allow_html=True)
            # NOVO COMPONENTE: Chave de liga/desliga para revelar a senha com total segurança
            mostrar_senha = st.toggle("👁️ Mostrar caracteres digitados", value=False)
            
            if mostrar_senha:
                # Se ativado, gera o campo com texto aberto
                senha = st.text_input("Senha", placeholder="Sua senha de acesso")
            else:
                # Se desativado, envelopa o campo na nossa div de mascaramento por bolinhas
