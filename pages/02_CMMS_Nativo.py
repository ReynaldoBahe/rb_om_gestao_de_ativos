...
# 📋 INICIALIZAÇÃO DE CHAVES DE MEMÓRIA PARA LIMPEZA AUTOMÁTICA DO FORMULÁRIO
if 'txt_id' not in st.session_state: st.session_state['txt_id'] = "29e456..."
if 'txt_sintoma' not in st.session_state: st.session_state['txt_sintoma'] = ""
if 'txt_manual' not in st.session_state: st.session_state['txt_manual'] = ""
...
with col1:
    nova_os = st.text_input("Código da OS (Automático)", value=f"OS-2026-{len(df) + 1:03d}", disabled=True)
    id_bim = st.text_input("ID BIM do Ativo (Speckle)", key='txt_id')
...
if st.button("💾 Registrar OS no Sistema"):
